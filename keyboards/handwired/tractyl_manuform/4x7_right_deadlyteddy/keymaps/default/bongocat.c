...
#define BONGO_OFFSET 20

static void oled_write_raw_P_offset(const char *data, uint16_t size, uint8_t offset) {
    for (uint8_t page = 0; page < 4; page++) {
        uint16_t page_start = page * 128;
        for (uint8_t i = 0; i < offset; i++) {
            oled_write_raw_byte(0, page_start + i);
        }
        for (uint8_t i = 0; i < 128 - offset && (page_start + i) < size; i++) {
            oled_write_raw_byte(pgm_read_byte(&data[page_start + i]), page_start + offset + i);
        }
    }
}
...
void render_bongocat(void) {
    ...
    void animation_phase(void) {
        if (get_current_wpm() <= IDLE_SPEED) {
            ...
            oled_write_raw_P_offset(idle[abs((IDLE_FRAMES - 1) - current_idle_frame)], ANIM_SIZE, BONGO_OFFSET);
        }

        if (get_current_wpm() > IDLE_SPEED && get_current_wpm() < ANIM_WPM_LOWER) {
            oled_write_raw_P_offset(prep[0], ANIM_SIZE, BONGO_OFFSET);
        }

        if (get_current_wpm() >= ANIM_WPM_LOWER) {
            current_tap_frame = (current_tap_frame + 1) % TAP_FRAMES;
            oled_write_raw_P_offset(tap[abs((TAP_FRAMES - 1) - current_tap_frame)], ANIM_SIZE, BONGO_OFFSET);
        }
    }
